...
int CAPECompress::Start(const char * pOutputFilename, const WAVEFORMATEX * pwfeInput, unsigned int nMaxAudioBytes, intn nCompressionLevel, const void * pHeaderData, intn nHeaderBytes)
{
...
int CAPECompress::AddData(unsigned char * pData, intn nBytes)
{
    ...
    while (nBytesDone < nBytes)
    {
		...
		intn nBytesToProcess = std::min(nBytesAvailable, nBytes - nBytesDone);
...
int CAPECompress::ProcessBuffer(bool bFinalize)
{
    ...
    try
    {
        ...
        {
            intn nFrameBytes = std::min(m_spAPECompressCreate->GetFullFrameBytes(), m_nBufferTail - m_nBufferHead);
